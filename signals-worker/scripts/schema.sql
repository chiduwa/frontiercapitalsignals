...
CREATE TABLE IF NOT EXISTS surge_signal_log (
  ...
  base_rate REAL,
  market_n INTEGER,
  -- The per-coin calibrated features the cast was made on (migration 0052).
  vol_z REAL,
  bar_z REAL,
  run24_z REAL,
  liquidity_30d REAL,
...
CREATE TABLE IF NOT EXISTS surge_config_status (
  ...
  -- at discovery: exhaustion20, and exhaustion_calibrated (2026-09-26).
...
-- Migration 0046: prospective freeze ledger for session/calendar timing rules.
...
-- One row per live scan: the market-wide exhaustion reading (migration 0052,
-- docs/research-2026-09-26/EXHAUSTION.md). Display and research only: a
-- market-wide volume surge in a rally has been followed by MORE upside.
