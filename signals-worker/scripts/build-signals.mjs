...
if (FCS_D1_DATABASE_ID) {
  try {
    ...
    techniquePriors = await loadTechniquePriors(env);
    console.log(`loaded technique priors for ${Object.keys(techniquePriors.byAssetClass.crypto || {}).length} crypto and ${Object.keys(techniquePriors.byAssetClass.stock || {}).length} stock techniques`);
    comboReliability = await loadComboReliability(env);
    console.log(`loaded combo reliability for ${Object.keys(comboReliability).length} (symbol, technique-pair) records`);
...
if (FCS_D1_DATABASE_ID) {
  ...
  try {
    // Same already-flowing asset_price_log this run's own logRun call
    // (above) just wrote to -- no new fetch.
    const suddenMoveAlerts = await checkAndNotifySuddenMoves(env, payload.generated_at);
