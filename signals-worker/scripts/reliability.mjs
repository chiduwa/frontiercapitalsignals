...
export async function loadLongTermBottomStatus(env) {
  const rows = await d1(env, 'SELECT symbol, low_close, low_date, days_since_low, current_close, pct_above_low, drawdown_pct FROM long_term_bottom_status');
  const out = {};
  for (const r of rows) out[r.symbol] = { lowClose: r.low_close, lowDate: r.low_date, daysSinceLow: r.days_since_low, currentClose: r.current_close, pctAboveLow: r.pct_above_low, drawdownPct: r.drawdown_pct };
  return out;
}
